...
export class SearchInput{
    ...
    constructor(element){
        ...
        this.start_conversation_base_url = this.resultBlockEl.attributes.startConversationUrl.value;
        this.follow_user_base_url = this.resultBlockEl.attributes.followUserUrl.value;
        /* This variable hold the id of the current search request: it is the id of a timeout process */
        this.currentSearchTimeoutId;
        /** The button the user have to trigger in order to follow a user */
        this.followButtonEl = this.resultBlockEl.querySelector('.toggle-follow-button');

        /** The button the user have to trigger in order to chat with a user */
        this.chatButtonEl = this.resultBlockEl.querySelector('.chat-button');
        this.init();
    }
    ...
    init(){
        ...
        this.resultBlockEl.addEventListener('click', (event) => this.execute.bind(this)(event));
    }
    ...
    load(data){
        ...
        data.forEach(user_infos => {
            ...
            result.setAttribute('userId', user_infos['id'])
            result.setAttribute('followed', user_infos['followed'])
            result.innerHTML = `
                <div class="avatar">
                ...
                <p>
                    <span style='color: rgb(255, 102, 0);'>@</span><span class='result-username'>${user_infos['username']}</span>
                    <button class='button chat-button'>chat</button>
                    <button class='button toggle-follow-button'>${user_infos['followed']? 'unfollow':'follow'}</button>
                </p>
            `;
    ...
    /** 
     * Executes user action: find if a user want to chat or follow the targetted user
     * @param {Event} event The event triggered in the DOM
     */
    execute(event){
        let target = event.target;
        if(target.classList.contains('chat-button')){
            let resultEl = target.closest('.result');
            this.startConversation(resultEl);
        }
        else if(target.classList.contains('toggle-follow-button')){
            let resultEl = target.closest('.result');
            this.toggleFollowUser(resultEl);
        }
    }

    /**
     * Starts a new conversation
     * @param {HTMLElement} target The targetted result block element that represents the user to chat with
     */
    startConversation(target){
        ...
        const other_user_id = target.attributes.userId.value;
        
        // Ask the server to start a new conversation
        const endpoint = `${this.start_conversation_base_url}${other_user_id}/`;
        fetch(endpoint, {
            method: 'post',
            headers: {
                'X-CSRFToken': cookies.csrftoken, 
    ...
    /**
     * Handles the process of following/unfollowing a user.
     * @param {HTMLElement} target The targetted result block element that represents the user to follow/unfollow
     */
    toggleFollowUser(target){
        // Retrive the state: followed or not followed
        const followed = target.attributes.followed.value;
        if(followed == 'false')
            this.followUser(target);
        else    
            this.unfollowUser(target);
    }

    /**
     * Handles the process of following a user.
     * @param {HTMLElement} target The targetted result block element that represents the user to follow
     */
    followUser(target){
        // Retrive the other user username
        const other_user_id = target.attributes.userId.value;
        ...
        // Ask the server to start a new conversation
        const endpoint = `${this.follow_user_base_url}${other_user_id}/`;
        fetch(endpoint, {
            method: 'POST',
            headers: {
                'X-CSRFToken': cookies.csrftoken, 
        ...
        .then(data => {
            ...
            // Continue if no error field is detected on the response: we have to toggle the follwed attribute from 'false' to 'true'
            target.setAttribute('followed', 'true'); // attributes receive strings, we do not need to give it a boolean then.
            target.querySelector('.toggle-follow-button').innerText = 'unfollow';
        })
        .catch(error => console.log(error));
    }
     ...
     * @param {HTMLElement} target The targetted result block element that represents the user to unfollow
     */
    unfollowUser(target){
        // Retrive the other user username
        const other_user_id = target.attributes.userId.value;
        ...
        .then(data => {
            ...
            target.setAttribute('followed', 'false'); // attributes receive strings, we do not need to give it a boolean then.
            target.querySelector('.toggle-follow-button').innerText = 'follow';
        })
        .catch(error => console.log(error));
    }
}
...
export class Post {
    ...
    constructor(post) {
        ...
        /** The button used to follow or unfollow an user */
        this.toggleFollowButtonEl = post.querySelector('.toggle-follow-button');
    ...
    init(){
        ...
        if(this.deletePostButton)  // Post on the explore page do not have this button, so we have to check the existence first
            this.deletePostButton.addEventListener("click", this.delete.bind(this));

        if(this.toggleFollowButtonEl) // Post on home page the section do not have this button, so we have to check the existence first
            this.toggleFollowButtonEl.addEventListener('click', this.toggleFollowUser.bind(this));
    }
    ...
    toggleFollowUser(){
        // Retrive the state: followed or not followed
        const followed = this.toggleFollowButtonEl.attributes.followed.value;
        if(followed == 'false')
            this.followUser();
        else    
            this.unfollowUser();
    ...
    followUser(){
        // Ask the server to start a new conversation
        const endpoint = this.toggleFollowButtonEl.attributes.followUrl.value;
        fetch(endpoint, {
            method: 'POST',
            headers: {
                'X-CSRFToken': cookies.csrftoken, 
        ...
        .then(data => {
            ...
            // Continue if no error field is detected on the response: we have to toggle the follwed attribute from 'false' to 'true'
            this.toggleFollowButtonEl.setAttribute('followed', 'true'); // attributes receive strings, we do not need to give it a boolean then.
            this.toggleFollowButtonEl.innerText = 'unfollow';
        })
        .catch(error => console.log(error));
    }
    ...
    /**
     * Handles the process of unfollowing a user.
     * @param {HTMLElement} target The targetted post element that was published by the user to unfollow
     */
    unfollowUser(){
       // Ask the server to start a new conversation
        const endpoint = this.toggleFollowButtonEl.attributes.followUrl.value;
        ...
        .then(data => {
            ...
            this.toggleFollowButtonEl.setAttribute('followed', 'false'); // attributes receive strings, we do not need to give it a boolean then.
            this.toggleFollowButtonEl.innerText = 'follow';
        })
        .catch(error => console.log(error));
    }
...
// (😭😭 I was experimenting this classs, but something make me give it up: look it youself, I do not know where is the problem)
// /**
